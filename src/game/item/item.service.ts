    ...
    switch (itemType.use) {
      ...
      case 'ball':
        ...
        if (this.useBall(trainer, itemType, monster)) {
          ...
          itemType.effects && this.monsterService.applyEffects(monster, itemType.effects);
        }
        break;
      case 'effect':
        if (!monster) {
        ...
        if (itemType.effects && !this.monsterService.applyEffects(monster, itemType.effects)) {
          throw new ForbiddenException('This item has no effect on this monster');
        }
        break;
    }
